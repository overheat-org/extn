...
import { Decorator, Container, Target } from './base';
...
export const injectable = Decorator.create({
    name: 'injectable',
...
export const manager = Decorator.create({
    name: 'manager',
    targets: [Target.Class],
    async transform(ast) {
...
export const http = Container.create({
    name: "http",
    errors: {
        UNKNOWN_DECORATOR: "invalid http method",
    },
    decorators: (() => {
        const base = Decorator.create({
            name: "base",
        ...
        return HTTP_METHODS.map(m => ({ ...base, name: m }) as Decorator);
    })()
});

export const event = Decorator.create({
    name: "event",
    targets: [Target.Method],
    transform(ast) {
...
export const serializable = Decorator.create({
    name: 'serializable',
    targets: [Target.Class],
    transform(ast) {

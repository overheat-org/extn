...
import { basename, extname, isAbsolute, normalize, relative, resolve, sep } from 'path';
...
class ModuleResolver {
  ...
  static resolvePath(path: string, parentPath?: string) {
    if(isAbsolute(path) && !parentPath) return this.resolveFile(path, this.extensions);
  ...
  static resolveFile(path: string, exts: string[]) {
    ...
    if (existsSync(path)) {
      ...
      return this.resolveExt(path + sep + "index", exts);
    }
  ...
  static pathToRelative(path: string, parentPath: string) {
    let relativePath = relative(parentPath, parentPath);
